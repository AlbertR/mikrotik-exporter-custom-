...
func (c *wlanSTACollector) init() {
	...
	for _, p := range c.props[len(c.props)-3:] {
		c.descriptions["tx"+p] = descriptionForPropertyName("wlan_station", "tx_"+p, labelNames)
		c.descriptions["rx"+p] = descriptionForPropertyName("wlan_station", "rx_"+p, labelNames)
	}
}
...
func (c *wlanSTACollector) collectMetricForProperty(property, iface, mac string, re *proto.Sentence,
...
func (c *wlanSTACollector) collectMetricForTXRXCounters(property, iface, mac string, re *proto.Sentence,
	ctx *collectorContext) {
	...
	if err != nil {
		log.WithFields(log.Fields{
			...
			"value":    re.Map[property],
			"error":    err,
		}).Error("error parsiing wlan station TXRX metric value")
		return
	}

	desc_tx := c.descriptions["tx_"+property]
	desc_rx := c.descriptions["rx_"+property]
	ctx.ch <- prometheus.MustNewConstMetric(desc_tx, prometheus.GaugeValue, tx, ctx.device.Name, ctx.device.Address,
		iface, mac)
	ctx.ch <- prometheus.MustNewConstMetric(desc_rx, prometheus.GaugeValue, rx, ctx.device.Name, ctx.device.Address,
		iface, mac)
}
