...
func (c *dhcpCollector) init()  {
		...
		c.leasesActiveCountDesc = description(prefix, "leases_active_count",
			"nuber of active leases per DHCP server", labelNames)
}
...
func (c *dhcpCollector) collect(ctx *collectorContext) error {
		...
		for _, n := range names {
				err := c.collectForDHCPServer(ctx, n)
				if err != nil {
						return err
				}
		}
...
func (c *dhcpCollector) collectForDHCPServer(ctx *collectorContext, dhcpServer string) error {
		reply, err := ctx.client.Run("/ip/dhcp-server/lease/print", fmt.Sprintf("?server=%s", dhcpServer),
			"=active=", "=count-only=")
		if err != nil {
				log.WithFields(log.Fields{
		...
		ctx.ch <- prometheus.MustNewConstMetric(c.leasesActiveCountDesc, prometheus.GaugeValue, v, ctx.device.Name,
			ctx.device.Address, dhcpServer)
		return nil
}
